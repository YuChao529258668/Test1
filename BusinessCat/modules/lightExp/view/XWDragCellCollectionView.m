...
- (void)xwp_gestureBegan:(UILongPressGestureRecognizer *)longPressGesture{
    ...
    if (self.longPressGesture.minimumPressDuration == 1) {
      [self xw_enterEditingModel];
      if ([self.delegate respondsToSelector:@selector(dragCellCollectionViewEditing:)]) {
        [self.delegate dragCellCollectionViewEditing:self];
      }
    }
...
//    if ([cells[i] isKindOfClass:[SubCollectionViewCell class]]) {
//      SubCollectionViewCell *cell = cells[i];
//      /**如果加了shake动画就不用再加了*/
...
//      }
//    }else{
//      SubMoreCollectionViewCell *cell = cells[i];
