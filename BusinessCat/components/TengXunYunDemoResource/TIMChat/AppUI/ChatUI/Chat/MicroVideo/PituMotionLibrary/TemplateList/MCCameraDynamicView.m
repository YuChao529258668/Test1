...
- (void)layoutSubviews {
    ...
    if (![MaterialManager isOnlinePackage:_materialID]) {
        ...
        BOOL exists = [[NSFileManager defaultManager] fileExistsAtPath:[[SDImageCache sharedImageCache] defaultCachePathForKey:[[SDWebImageManager sharedManager] cacheKeyForURL:iconURL]]];
        if (exists) {
            [self startAnimation:NO];
        } else {
