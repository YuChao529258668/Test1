...
//- (UIWebView *)webView {
//    if (!_webView) {
//        _webView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 20, SCREEN_WIDTH, SCREEN_HEIGHT-64)];
//    }
//    return _webView;
//}
...
-(void)getData{
    ...
    [self.biz queryRemoteInfoDetailById:self.infoId type:self.type success:^(CGInfoDetailEntity *infoDetail) {
        ...
        if (infoDetail.pageCount>0&&![infoDetail.infoPic hasSuffix:@".svg.png"]) {
            ...
            NSString *urlStr = [infoDetail.fileUrl stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
...
-(void)detailToolbarShareAction{
    ...
    if (self.detailType == 4) {
        desc = @"我在沟通猫发现了这个非常优质的岗位工具，现在也推荐给你";
    }else if (self.type == 1){
        desc = @"我在沟通猫发现了这份非常优质的知识，现在也推荐给你";
    }else{
        desc = @"我在沟通猫发现了这份非常优质的文档，现在也推荐给你";
    }
...
-(UIImageView *)hudIV{
    if (!_hudIV) {
        ...
        NSURL *url = [[NSBundle mainBundle] URLForResource:@"juhua" withExtension:@"gif"];
