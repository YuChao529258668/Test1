...
//        return NO;
//    }
    // Thread 1: EXC_BAD_ACCESS (code=10, address=0x108ef3000)
    if (Mtc_Init() == ZOK) {
        NSLog(@"电话语音初始化成功 %@", NSStringFromSelector(_cmd));
    }

    [MtcLoginManager Init:kJCKey];
    //    [MtcPushManager SetPayloadForCall:@MY_PUSH_CALL_PAYLOAD                     Expiration:MY_PUSH_EXPIRATION]; // 推送
    [MtcCallManager Init];
