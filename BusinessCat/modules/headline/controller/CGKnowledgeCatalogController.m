...
//  url = [self encodeString:url];
  ...
  self.shareUtil = [[ShareUtil alloc]init];
  [self.shareUtil showShareMenuWithTitle:self.dataList.title desc:@"我在荐识发现了这份非常优质的专辑，现在也推荐给你" isqrcode:1 image:self.dataList.cover url:url block:^(NSMutableArray *array) {
    UIActivityViewController *activityVC = [[UIActivityViewController alloc]initWithActivityItems:array applicationActivities:nil];
